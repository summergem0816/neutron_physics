...
def main():
    ...
    diagnostics_lines = [
        ...
        "readout_sigma\tscatter_strength\tmean_to_sample_abs\tmean_to_sample_std\tmean_to_learned_abs\tmean_to_learned_std"
    ]
    ...
    with torch.no_grad():
        for image_path in image_paths:
            ...
            for key in LEVEL_KEYS:
                ...
                learned_delta_abs = 0.0
                learned_delta_std = 0.0
                if "degraded_learned" in mean_out:
                    learned_degraded = mean_out["degraded_learned"]
                    _save_image_minus1_1(learned_degraded, final_dir / f"{sample_prefix}_pred_{suffix}_learned_noise.png", original_size)
                    learned_delta = _crop_to_size(learned_degraded - mean_degraded, original_size)
                    _save_feature_grid(learned_delta, final_dir / f"{sample_prefix}_learned_noise_residual_{suffix}_grid.png")
                    learned_delta_abs = float(learned_delta.abs().mean().item())
                    learned_delta_std = float(learned_delta.std(unbiased=False).item())
                ...
                level_meta = {
                    ...
                    "mean_to_learned_abs": learned_delta_abs,
                    "mean_to_learned_std": learned_delta_std,
                }
                ...
                diagnostics_lines.append(
                    ...
                    f"{learned_delta_abs:.6g}\t{learned_delta_std:.6g}"
                )
