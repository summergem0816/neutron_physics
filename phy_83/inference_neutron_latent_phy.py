...
def main():
    ...
    autoencoder, physics_forward, _, checkpoint_path, t_map = load_stage1_modules(
        model_cfg,
        project_root=project_root,
        device=device,
    ...
    with torch.no_grad():
        for group in groups:
            ...
            _, control_points = build_control_points(autoencoder, sample)
            ...
            for index, (alpha, t_query) in enumerate(query_times, start=1):
                ...
                t_tensor = t_query.view(1).expand(sample["GT"].shape[0]).to(device=sample["GT"].device, dtype=sample["GT"].dtype)
                pred_img = physics_forward(
                    clean=sample["GT"],
                    t=t_tensor,
                ...
                save_image(
                    ...
                    output_dir / f"interp_physics_{args.start_key}_to_{args.end_key}_a{int(round(alpha * 100)):02d}.png",
                )

    print(f"Saved physics interpolation results to {Path(args.output).resolve()}")
