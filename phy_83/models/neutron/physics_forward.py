...
class LearnedNoiseUNet(nn.Module):
    def __init__(
        self,
        image_channels: int,
        latent_channels: int,
        ...
        super().__init__()
        in_channels = image_channels * 3 + latent_channels + 4
        self.latent_channels = int(latent_channels)
        self.residual_scale = float(residual_scale)

        self.enc1 = _ResidualConvBlock(in_channels, hidden_dim)
        self.down1 = nn.Sequential(
            nn.Conv2d(hidden_dim, hidden_dim * 2, kernel_size=3, stride=2, padding=1),
            nn.GELU(),
        )
        self.enc2 = _ResidualConvBlock(hidden_dim * 2, hidden_dim * 2)
        self.down2 = nn.Sequential(
            nn.Conv2d(hidden_dim * 2, hidden_dim * 4, kernel_size=3, stride=2, padding=1),
            nn.GELU(),
        )
        self.bottleneck = _ResidualConvBlock(hidden_dim * 4, hidden_dim * 4)
        self.dec2 = _ResidualConvBlock(hidden_dim * 6, hidden_dim * 2)
        self.dec1 = _ResidualConvBlock(hidden_dim * 3, hidden_dim)
    ...
    def forward(
        self,
        clean_01: torch.Tensor,
        degraded_mean_01: torch.Tensor,
        pre_poisson: torch.Tensor,
        degradation_latent: torch.Tensor | None,
        t: torch.Tensor,
        ...
        batch, _, height, width = degraded_mean_01.shape
        if degradation_latent is None:
            latent_up = torch.zeros(
                batch,
                self.latent_channels,
                height,
                width,
                device=degraded_mean_01.device,
                dtype=degraded_mean_01.dtype,
            )
        else:
            latent_up = F.interpolate(degradation_latent, size=(height, width), mode="bilinear", align_corners=False)
        t_map = t.view(-1, 1, 1, 1).expand(-1, 1, height, width)
        ...
        deep = self.bottleneck(self.down2(enc2))
        ...
        residual = self.head(dec1) * self.residual_scale
        return residual.repeat(1, degraded_mean_01.shape[1], 1, 1)
...
class NeutronPhysicalForward(nn.Module):
    def __init__(
        ...
        learned_noise_hidden_dim: int = 32,
        learned_noise_residual_scale: float = 0.25,
    ):
        super().__init__()
        ...
        self.use_learned_noise = bool(use_learned_noise)
        ...
        self.learned_noise = LearnedNoiseUNet(
            image_channels=3,
            latent_channels=latent_channels,
            hidden_dim=learned_noise_hidden_dim,
            residual_scale=learned_noise_residual_scale,
        )
        if not self.use_learned_noise:
            for param in self.learned_noise.parameters():
    ...
    def forward(
        ...
        poisson_mean = self.apply_poisson_layer(
            pre_poisson,
            source_stats["particle_count_effective"],
            sample_noise=False,
        )
        poisson_mean = self._repeat_gray_channels(poisson_mean)
        mean_out, readout_sigma = self.apply_readout_noise(
            poisson_mean,
            t,
            cond["readout_map"],
            sample_noise=False,
        )
        mean_out = self._repeat_gray_channels(mean_out)
        degraded_mean = mean_out.clamp(0.0, 1.0).mul(2.0).sub(1.0)
        ...
        noisy_out, _ = self.apply_readout_noise(
            poisson_out,
            t,
            cond["readout_map"],
            sample_noise=sample_noise,
        ...
        if self.use_learned_noise:
            learned_noise_residual = self.learned_noise(
                clean_01=source_stats["transmission"],
                degraded_mean_01=mean_out.clamp(0.0, 1.0),
                pre_poisson=pre_poisson.clamp(0.0, 1.0),
                degradation_latent=degradation_latent,
                t=t,
            ...
            learned_noise_residual = torch.zeros_like(mean_out)
        learned_out = (mean_out + learned_noise_residual).clamp(0.0, 1.0)
        degraded_learned = learned_out.mul(2.0).sub(1.0)
        ...
        return {
            ...
            "learned_noise_residual": learned_noise_residual,
            "degraded_learned": degraded_learned,
