...
class LitZOnlyDegradation(pl.LightningModule):
    def __init__(
        ...
        self.consistency_loss = DegradationConsistencyLoss(
            ...
            z_highfreq_loss_scale=float(zonly_config.get("z_highfreq_loss_scale", 0.0)),
    ...
    def _shared_step(self, batch: dict[str, torch.Tensor], mode: str) -> torch.Tensor:
        ...
        loss_parts = {
            ...
            "z_highfreq": torch.zeros((), device=clean.device),
        ...
        for key in LEVEL_KEYS:
            ...
            if self.physics_forward is not None:
                ...
                pred_img = physics_out.get("degraded_mean", physics_out["degraded"])
                outputs_by_level[key] = physics_out
                pred_sample = None
                if self.consistency_loss.needs_noise_sample:
                    if "degraded_learned" in physics_out:
                        pred_sample = physics_out["degraded_learned"]
                    else:
        ...
        log_dict = {
            ...
            f"{log_prefix}/z_highfreq": loss_parts["z_highfreq"],
        ...
        if mode == "val":
            for key in LEVEL_KEYS:
                pred_tensor = outputs_by_level[key].get("degraded_learned", outputs_by_level[key].get("degraded_mean", outputs_by_level[key]["degraded"]))
                pred = pred_tensor.clamp(-1, 1).add(1).div(2)
